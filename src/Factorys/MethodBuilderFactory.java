...
import Builder.PMAdaptiveLocalSearchBuilder;
import Builder.PMAdaptiveLocalSearchBuilder_OLD;
import Builder.PMConstructionBuilder;
import Builder.PMLocalSearchBuilder;
...
import ILS_PM.PMILSStopConditionTime;
			...
			case "pmp":
				switch (parameters.getMthType()) {
					case "ils":
						...
						ILSBuilder pm_ils_builder = null;
						if(builder instanceof ILSBuilder) {
							pm_ils_builder = (ILSBuilder) builder;
						}
						
						MethodBuilder construct_builder = new PMConstructionBuilder();
						construct_builder.setFunctionType(parameters.getConstrType());
						pm_ils_builder.setConstructBuilder(construct_builder);
						...
						switch (parameters.getLsType()) {
							case "basic_vnd_complete":
								System.out.println("VNS Classic");
								ls_builder = new PMLocalSearchBuilder();
								break;
							case "adaptive_vnd_complete":
								ls_builder = new PMAdaptiveLocalSearchBuilder();
								break;
							/*case "new_adaptive_vnd":
								ls_builder = new PMNewAdaptiveLocalSearchBuilder();
								break;
							*/
							case "adaptive_vnd_old":
								ls_builder = new PMAdaptiveLocalSearchBuilder_OLD();
								break;
							default:
								System.out.println("\nTipo de Busca Local não disponível!");
								break;
						...
						ls_builder.setFunctionType(parameters.getLsType());
						pm_ils_builder.setLsBuilder(ls_builder);
						
						switch (parameters.getStopCondType()) {
							case "iterations":
								pm_ils_builder.getIls().setCondition(new PMILSStopConditionIterations());
								break;
							case "time":
								pm_ils_builder.getIls().setCondition(new PMILSStopConditionTime());
								break;
							default:
								System.out.println("\nTipo de Condição de Parada não disponível!");
								break;
						...
						pm_ils_builder.getIls().setPerturb(new PMILSPerturbationInLevels());
								
						parameters.setAcceptCritType("fo");
						pm_ils_builder.getIls().setCriteria(new PMILSAcceptationCriteriaOF());
