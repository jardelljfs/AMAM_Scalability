...
import Environment_VRP_Movements.VRPMovementElement;
import Parameters.Parameters;
import VNS.VNSGeneratesNeighbor;

public class VRPVNSGeneratesNeighbor implements VNSGeneratesNeighbor{

	@Override
	public Solution generatesNeighbor(int k, Solution s, Element e, int id_agent, Cooperation cooperation,
			Parameters parameters, Problem p) {
		// TODO Auto-generated method stub
		
        VRPSolution sol_neighbor = new VRPSolution(parameters.getExperimentParameters().getReceiverSize(), p);
		...
		sol_neighbor.copyValuesSolution(sol_vrp, p);
		
		switch (k) {
			case 1: 
				//deslocamento inter-máquina de duas tarefas aleatórias 
				sol_neighbor.copyValuesSolution(this.perturbationTwoShiftInterRandom(sol_neighbor, parameters, p), p);
				break;
			case 2:
				sol_neighbor.copyValuesSolution(this.perturbationTwoSwapInterRandom(sol_neighbor, parameters, p), p);
				break;
			case 3:
				//deslocamento inter-máquina de três tarefas aleatórias
				sol_neighbor.copyValuesSolution(this.perturbationThreeShiftInterRandom(sol_neighbor,parameters, p), p);
				break;
			case 4:
				sol_neighbor.copyValuesSolution(this.perturbationThreeSwapInterRandom(sol_neighbor, parameters, p), p);
				break;
			case 5:
				sol_neighbor.copyValuesSolution(this.perturbationThreeShifInterConsecutive(sol_neighbor, parameters, p), p);
				break;
			case 6:
				sol_neighbor.copyValuesSolution(this.perturbationEliminatesSmallRoute(sol_neighbor, parameters, p), p);
				if(sol_vrp.getNumberRoutes() < sol_neighbor.getNumberRoutes())
					System.out.println("\nDIMINUIU O NÚMERO DE ROTAS!");
				break;
			case 7:
				sol_neighbor.copyValuesSolution(this.perturbationEliminatesRandomRoute(sol_neighbor, parameters, p), p);
				if(sol_vrp.getNumberRoutes() < sol_neighbor.getNumberRoutes())
					System.out.println("\nDIMINUIU O NÚMERO DE ROTAS!");
				break;
			case 8:
				long solution_time = System.currentTimeMillis() - parameters.getMethodParametersI(id_agent).getInitialTime();
				sol_neighbor.copyValuesSolution(cooperation.getPool().getRandomSolutionCollection(id_agent, solution_time), p);
				Receiver receiver = new Receiver(parameters.getExperimentParameters().getReceiverSize());
				sol_neighbor.setReceiver(receiver);
				break;
			default:
				System.out.println("\n\nInvalid Neighbor Function!");
		}
