...
public class IteratedLocalSearch extends Method{
	...
	public Solution runMethod(Problem p, Solution s, int id_agent) {
		...
		this.getCooperation().getPool().addSolutionInPool_evaluation(this.so, p, id_agent, solution_time, this.element);
		...
		while(this.condition.stopCondition(this.element, p, this.best_solution)) {
			...
			this.perturb_solution.copyValuesSolution(this.perturb.perturbation(this.s, this.element, id_agent, this.getCooperation(), this.getParameters(), p), p);
			//System.out.print("\n\n--PERTURB SOLUTION--");
		...
		this.getParameters().getMethodParametersI(id_agent).setBestMethodSolution(this.best_solution);
