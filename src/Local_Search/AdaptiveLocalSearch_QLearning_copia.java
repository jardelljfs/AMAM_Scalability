...
public class AdaptiveLocalSearch_QLearning_copia extends Method{
	...
	public Solution runMethod(Problem p, Solution so, int id_agent) {
		...
		//used to evaluate states already visited
		...
		while (improved) {
			...
			while (episode) {
				...
				if(no_improvement == 0) {
					//Reinforcement Learning - 1 --> epsilon greedy function
					...
					//Reinforcement Learning - 2 --> random function
				...
				if(criteria.acceptationCriteria(this.s_line, this.best_solution, this.element, p)) {
					...
					reward = 20; // reward + 20;//this.s_line.getFitnessLearning();
					//System.out.println("\n\nReward: " + reward);
